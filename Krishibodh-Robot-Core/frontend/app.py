...
import os
import threading  # <--- NEW: For background listening
import json # <--- NEW: For JSON storage
# import database  # <--- Removed DB module

# --- Initialize Flask App ---
app = Flask(__name__)
app.secret_key = 'krishibodh-is-a-super-secret-project-98765'

# --- Initialize Database (NOT USED ANYMORE) ---
...
ser = None
stop_thread = False  # <--- NEW: Flag to stop background thread
# This list is the persistent log. It will not clear until the server restarts.
message_log_history = []
last_reading_time = 0  # <--- NEW: For rate limiting readings
should_save_moisture = False # <--- NEW: Flag to control when to save readings to file

# Absolute path for moisture readings file to ensure it's always in the frontend folder
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
MOISTURE_FILE = os.path.join(BASE_DIR, "moisture_readings.txt")
...
def persist_moisture_reading(value, action="CHECKED"):
    """Saves moisture reading to simple text file."""
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    
    # Save to moisture_readings.txt (using absolute path)
    try:
        with open(MOISTURE_FILE, "a") as f:
            log_entry = f"{timestamp} | Moisture: {value} | {action}\n"
            f.write(log_entry)
        print(f"✅ Saved moisture reading: {value}")
        log_message(f"💾 ({get_timestamp()}) Saved: Moisture {value} to file.", "success")
    except Exception as e:
        print(f"Error writing to moisture_readings.txt: {e}")
...
def serial_listener():
    """Background thread that continuously monitors Serial for any data."""
    global ser, stop_thread
    log_message("📡 Background Serial Listener Started.", "info")
    while not stop_thread:
        if ser and ser.is_open:
            try:
                if ser.in_waiting > 0:
                    read_arduino_response(timeout=0.1)
                else:
                    time.sleep(0.1)
            except Exception as e:
                # Log error but keep thread alive unless connection is dead
                if not ser or not ser.is_open:
                    break
                time.sleep(1)
        else:
            time.sleep(1)
    log_message("📡 Background Serial Listener Stopped.", "info")
...
def read_arduino_response(timeout=0.5):
    ...
    try:
        ...
        if ser.in_waiting > 0:
            ...
            # Check if response is a moisture value (just a number or contains a number)
            # We strip whitespace and check if it's numeric
            clean_response = response.strip()
            if clean_response and clean_response.isdigit():
                log_message(f"💧 ({get_timestamp()}) Moisture Value: {clean_response}", "info")
                
                # Check global flag - only save if explicitly requested
                global should_save_moisture
                if should_save_moisture:
                    persist_moisture_reading(response, "CHECKED")
                    should_save_moisture = False # Reset flag after saving
                    log_message(f"💾 ({get_timestamp()}) Reading saved based on active request.", "success")
...
def send_command_to_arduino(command_string, log_command=True):
    ...
    try:
        ...
        # NOTE: We no longer call read_arduino_response here because 
        # the background thread handles all reading automatically!
...
def controller():
    """Main Control Dashboard (formerly index)."""
    ...
    # Fetch latest readings for the dashboard from JSON
    try:
        if os.path.exists("readings.json"):
            with open("readings.json", "r") as f:
                db_readings = json.load(f)
                # Reverse to get latest first in the table
                db_readings = list(reversed(db_readings))
                # Limit to 20 for the UI
                db_readings = db_readings[:20]
        else:
            db_readings = []
    except Exception as e:
        print(f"Error loading readings.json: {e}")
        db_readings = []
    
    # Pass the entire log history to the webpage
    # Note: We now render 'controller.html' instead of 'index.html'
    return render_template('controller.html', 
                           available_ports=available_ports,
                           is_connected=is_connected,
                           connected_port=connected_port,
                           log_history=message_log_history,
                           db_readings=db_readings) 
...
def ai_command():
    ...
    if command_string.startswith("WATER1") or command_string.startswith("WATER2"):
        ...
        # New Firmware (v4.0) supports "MOVE:" prefix for Move Only (No Water check)
        # We pass the full command "MOVE:x,y,z" directly to Arduino
        final_command = command_string
        action_type = f"Manual Move (No Check)"

    elif command_string.startswith("SMART:"):
        # Smart Check Mode
        final_command = command_string
        action_type = "Smart Water Check"
        
    elif command_string.startswith("WATER:") or command_string.startswith("FORCE:"):
        # Force Water Mode - Normalize to WATER command for Arduino
        final_command = command_string.replace("FORCE:", "WATER:").replace("FORCE_WATER:", "WATER:")
        action_type = "Force Water"
...
# --- EXPERIMENT FLOW ROUTES ---

@app.route('/experiments')
def experiments_page():
    return render_template('experiments.html')

@app.route('/generate_experiments', methods=['POST'])
def generate_experiments():
    """
    Mock AI Endpoint. In a real scenario, this would call an LLM.
    For now, it returns 4 diverse, hardcoded plans based on the hypothesis.
    """
    ...
    plans = [
        {
            "id": 1,
            "title": "Aggressive Water Testing",
            "steps": [
                "Move to Plant 1",
                "Measure status",
                "Wait 10 seconds",
            ...
            "execution_logic": {
                "sequence": [
                    {"action": "move_plant_1", "wait_after": 2},
                    {"action": "check_sensor", "wait_after": 10},
                    {"action": "move_plant_2", "wait_after": 2}
                ]
            },
        ...
        },
        {
            ...
            "execution_logic": {
                "sequence": [
                    {"action": "home", "wait_after": 60},
                    {"action": "move_plant_1", "wait_after": 5},
        ...
        },
        {
            ...
            "execution_logic": {
                "sequence": [
                    {"action": "move_plant_1", "wait_after": 2},
                    {"action": "water_smart_1", "wait_after": 5},
                    {"action": "home", "wait_after": 2},
                    {"action": "move_plant_2", "wait_after": 2}
                ]
            },
        ...
        {
            "id": 4,
            "title": "Rapid Stress Test",
            "steps": [
                "Cycle Plant 1 -> Home -> Plant 2",
                "No actions, just movement",
                "Repeat"
            ],
            "execution_logic": {
                "sequence": [
                    {"action": "move_plant_1", "wait_after": 1},
                    {"action": "home", "wait_after": 1},
                    {"action": "move_plant_2", "wait_after": 1}
                ]
            },
...
@app.route('/save_experiment_plan', methods=['POST'])
def save_experiment_plan():
    """Saves the selected JSON plan to a file for the Bot to read."""
    try:
        data = request.get_json()
        plan = data.get('plan')
        
        # Default to INACTIVE so it doesn't auto-start
        plan['active'] = False
        
        # Save to the parent directory where the bot script will live
        file_path = os.path.join("..", "experiment_plan.json")
        
        with open(file_path, "w") as f:
            json.dump(plan, f, indent=4)
            
        log_message(f"🧪 ({get_timestamp()}) New Protocol Loaded: {plan.get('title')}. Status: PAUSED (Waiting for Start).", "success")
        return jsonify({"success": True})
    except Exception as e:
        log_message(f"❌ ({get_timestamp()}) Failed to save experiment plan: {e}", "error")
        return jsonify({"success": False, "message": str(e)}), 500

@app.route('/toggle_experiment', methods=['POST'])
def toggle_experiment():
    """Toggles the 'active' state of the current experiment plan."""
    try:
        data = request.get_json()
        desired_state = data.get('active', False) # True to start, False to stop
        
        file_path = os.path.join("..", "experiment_plan.json")
        
        if not os.path.exists(file_path):
             return jsonify({"success": False, "message": "No plan loaded."}), 404
             
        # Read, Update, Save
        with open(file_path, "r") as f:
            plan = json.load(f)
            
        plan['active'] = desired_state
        
        with open(file_path, "w") as f:
            json.dump(plan, f, indent=4)
            
        status_msg = "STARTED/RESUMED" if desired_state else "PAUSED/STOPPED"
        log_message(f"⏯️ ({get_timestamp()}) Experiment {status_msg} by user.", "info" if not desired_state else "success")
        
        return jsonify({"success": True, "active": desired_state})
    except Exception as e:
        log_message(f"❌ ({get_timestamp()}) Failed to toggle experiment: {e}", "error")
        return jsonify({"success": False, "message": str(e)}), 500

# --- END EXPERIMENT FLOW ROUTES ---
...
def disconnect():
    ...
    global ser, stop_thread
    stop_thread = True # Stop the listener
    if ser and ser.is_open:
        try:
            port = ser.port
            ser.close()
...
def move():
    ...
    # --- CONTROLLED SAVING: Enable saving for Plant moves ---
    global should_save_moisture
    if action in ['plant1', 'plant2', 'water1', 'water2']:
        should_save_moisture = True
        log_message(f"💾 ({get_timestamp()}) Expecting moisture reading... saving enabled.", "info")
    # -----------------------------------------------------
...
def test_z_axis():
    ...
    # Use MOVE: prefix to ensure we don't trigger water sensor check
    test_z_down = "MOVE:0,0,5000"  # Move Z down to 5000 steps (X=0, Y=0, Z=5000)
    test_z_up = "MOVE:0,0,0"       # Move Z back to 0
...
@app.route('/read_sensor_now', methods=['POST'])
def read_sensor_now():
    """Trigger an on-demand moisture reading and save it."""
    log_message(f"🩺 ({get_timestamp()}) Requesting manual moisture reading...", "info")
    
    # Enable saving for this specific request
    global should_save_moisture
    should_save_moisture = True
    
    if send_command_to_arduino("MOISTURE:GET"):
        log_message(f"✅ ({get_timestamp()}) Moisture request sent. Saving enabled.", "success")
        # We don't wait here anymore - background thread handles the saving!
    else:
        log_message(f"❌ ({get_timestamp()}) Failed to send moisture request.", "error")
        should_save_moisture = False # Reset if send failed
        
    return redirect(url_for('controller'))

@app.route('/test_sensor')
def test_sensor():
    """Diagnostic: Read sensor without moving."""
    log_message(f"🩺 ({get_timestamp()}) Testing Sensor Only...", "info")
    if send_command_to_arduino("SENSOR:CHECK"):
        log_message(f"✅ ({get_timestamp()}) Sensor command sent.", "info")
    else:
        log_message(f"❌ ({get_timestamp()}) Failed to send command.", "error")
    return redirect(url_for('controller'))

@app.route('/debug_db')
def debug_db():
    """Manually force a log entry to test the DB connection."""
    try:
        database.log_reading(999, "DEBUG_TEST", 123, "TEST_WRITE")
        log_message("✅ Debug: Manually wrote to Database!", "info")
    except Exception as e:
        log_message(f"❌ Debug DB Error: {e}", "error")
    return redirect(url_for('controller'))
